...
- `LoggingObject(level=...)` sets the global threshold (`DEBUG`/`INFO`/`WARNING`/`ERROR`). Deliberately capped at `ERROR` — both `LoggingObject.__init__` and `proc_frame._parse_log_level()` reject anything higher (e.g. `CRITICAL`) — errors must always be logged, so exceeding `ERROR` is a configuration error, not a silently-honored setting. INI `[logging] level` (case-insensitive) is **required** in full mode (config_file_path given) — no silent default (CLAUDE.md: no graceful degradation); missing or empty raises `ValueError`. There is no `error_only` parameter anymore — it was removed once `level=logging.ERROR` (or INI `[logging] level = ERROR`) fully subsumed it; a leftover `error_only` entry in `[logging]` now raises a clear error instead of being silently ignored. Removing `error_only` was a deliberate, one-time exception to the API-stability rule below
- `proc_frame_start(..., level=logging.INFO)` — this `level` parameter only takes effect in **console-only mode** (`config_file_path=None`); in full mode it's overwritten by the INI-derived value (same variable name, reassigned in that branch) and thus ignored. Default is `logging.INFO`, **not** `logging.DEBUG` — a deliberate console-only-mode behavior change (DEBUG messages are now suppressed by default in console-only mode, unlike before this parameter existed)
...
- **CSV log format:** `Timestamp;Application;Version;PID;ThreadID;ThreadName;Class;Method;Level;Message` (`CSV_LOG_HEADER` in `csv_log_formatter.py`) — `Level` (`DEBUG`/`INFO`/`WARNING`/`ERROR`) sits right before `Message`, populated from stdlib's `record.levelname`
...
**Required INI `[logging]` parameter:** `level` (`DEBUG`/`INFO`/`WARNING`/`ERROR`) — no default, must be explicit

## Known Limitations

- Internally backed by stdlib `logging` (Phase 1 of the migration, see `docs/internal/LOGGING_MIGRATION_KONZEPT.md`), with a per-call log-level family (`log_debug`/`log_info`/`log_warning`/`log_error`/`log_and_raise`, `log_msg(level=...)`) and a required, INI-configurable global level threshold (`[logging] level = DEBUG|INFO|WARNING|ERROR`, no default) on top of it. Still missing: log rotation, configurable output format, and per-module logger hierarchy — these remain planned for later migration phases
